    ...
    try
    {
        ...
        if (data != null && data.Read())
        {
            return new Expense(Convert.ToInt32(data["id"]))
            {
                ...
                CategoryId = (int)data["category_id"],
                CurrencyId = (int)data["currency_id"],
                Comments = data["comments"] as string ?? string.Empty,
                CreatedAt = Convert.ToDateTime(data["created_at"])
            };
        }
        return null;
...
";
    ...
    Console.WriteLine($"Expense received in POST: " +
    $"Id={s.Id}, UserId={s.UserId}, Amount={s.Amount}, CategoryId={s.CategoryId}, " +
    $"CurrencyId={s.CurrencyId}, expenseDate={s.ExpenseDate}, Comments='{s.Comments}', CreatedAt={s.CreatedAt}");
...
cmd.Parameters.AddWithValue("@expense_date", NpgsqlDbType.Date, createdAt);
